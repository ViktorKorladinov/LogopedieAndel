...
function generateEmailHtml(data: FormData): string {
    ...
    for (const [key, value] of data.entries()) {
        if (key !== "cf-turnstile-response" && value) {
            let displayKey = key;
            if (key === 'name') displayKey = 'Jméno a příjmení';
            if (key === 'email') displayKey = 'Kontaktní e-mail';

            emailHtml += `<tr><td style='width:35%; background:#f3f4f6;'><strong>${displayKey}</strong></td><td>${value}</td></tr>`;
        }
    }
    emailHtml += "</table>";
    return emailHtml;
